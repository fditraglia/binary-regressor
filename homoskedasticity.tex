...
\begin{thm}
  \label{thm:skew}
  Under Assumptions \ref{assump:homosked}-\ref{assump:skew} and the conditions of Theorem \ref{pro:Lack}, the mis-classification rates $\alpha_0$ and $\alpha_1$ and the treatment effect $\beta$ are identified provided that $\alpha_0 \neq \alpha_1$ and $z$ takes on at least two values.
\end{thm}
...
After simplifying, we obtain a quadratic with \emph{identical} coefficients, implying that the second root of Equation \ref{eq:quadratic} identifies $\alpha_0$.
Provided that $\alpha_0 \neq \alpha_1$, we learn the sign of the difference $(\alpha_1 - \alpha_0)$ from Theorem \ref{pro:homosked}.
This tells us which mis-classification rate is larger and hence allows us to correctly label the two roots.
Substituting into $\beta = \mathcal{W}(1-\alpha_0-\alpha_1)$ identifies the treatment effect.
\end{proof}

Note that, in contrast to other results in the literature \citep{KRS, BBS, FL, Mahajan, Lewbel}, this result does \emph{not} require the assumption that $\alpha_0 + \alpha_1 <1$ to identify $\beta$.
However, it does require that $\alpha_0 \neq \alpha_1$.
If, on the contrary, $\alpha_0 = \alpha_1 = \alpha$ the preceding result produces two candidates for $\alpha$, but we cannot tell which is the true mis-classification rate and which is its complement $1-\alpha$. 
In this case, $\beta$ is only identified up to sign.
Nevertheless, by imposing the additional assumption that $\alpha < 1/2$ we can distinguish the two solutions and hence identify the sign of the treatment effect. 
