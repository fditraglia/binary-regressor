...
\begin{align*}
  ...
  \end{array}
\right] = 
  \frac{1}{n}\sum_{i=1}^n g(W_i, \theta_0, \tau) \\
G_n(\theta_0, \tau) &= \left[
\begin{array}{c}
  M_n(\theta_0, \tau)\\
  H_n(\theta_0, \tau)
\end{array}
\right] = n^{-1}\sum_{i=1}^n \partial g(W_i, \theta_0, \tau)/\partial \tau'
\end{align*}
Note that we are holding $\theta$ \emph{fixed} at $\theta_0$.
Let $\widehat{\tau}_n(\theta_0)$ denote the following GMM estimator
\[
  \widehat{\tau}_n(\theta_0) = \underset{\tau \in \mathscr{T}}{\arg \min} \; \bar{h}_n(\theta_0, \tau)'\, \widehat{\Xi} \, \bar{h}_n(\theta_0, \tau)
\]
where $\widehat{\Xi} \rightarrow_p \Xi$.
By a mean-value expansion,
\[
  \bar{g}_n\left(\theta_0, \widehat{\tau}_n(\theta_0)\right) = \bar{g}_n(\theta_0, \tau(\theta_0)) + G_n(\theta_0, \widehat{\tau}_n(\theta_0), \tau(\theta_0), \lambda_n) \left( \widehat{\tau}_n(\theta_0) - \tau(\theta_0) \right)
\]
where the $i$th row of $G_n(\theta_0, \widehat{\tau}_n(\theta_0), \tau(\theta_0), \lambda_n)$ equals $i$th row of $G_n\left(\theta_0, \bar{\tau}_n(\theta_0)^{(i)}\right)$ and $\bar{\tau}_n(\theta_0)^{(i)} = \lambda_{n,i}\tau(\theta_0) + (1 - \lambda_{n,i}) \widehat{\tau}_n(\theta_0)$ for some $0 \leq \lambda_{n,i} \leq 1$.


\[
  \sqrt{n} \left( \widehat{\tau}_n(\theta_0) - \tau(\theta_0) \right) 
\]
