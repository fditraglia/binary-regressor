...
\[
  G_\vartheta = \left[
  \begin{array}{c}
    \mathbb{E}[\mathbf{w}_i z_i]' \frac{\partial \psi_1}{\partial \vartheta'}\\
    \mathbb{E}[\mathbf{w}_i z_i]' \frac{\partial \psi_2}{\partial \vartheta'}\\
    \mathbb{E}[\mathbf{w}_i z_i]' \frac{\partial \psi_3}{\partial \vartheta'}
  \end{array}
\right], \quad
  H_\vartheta = \left[
  \begin{array}{c}
    \mathbb{E}[\mathbf{w}_i]' \frac{\partial \psi_1}{\partial \vartheta'}\\
    \mathbb{E}[\mathbf{w}_i]' \frac{\partial \psi_2}{\partial \vartheta'}\\
    \mathbb{E}[\mathbf{w}_i]' \frac{\partial \psi_3}{\partial \vartheta'}
  \end{array}
\right]
\]
...
\section{GMS for Frazis and Loewenstein / Mahajan / BBS}
We still use the same moment inequalities as above as these continue to hold under the assumptions of Frazis \& Loewenstein etc.
However, the moment equalities change.
It's more convenient to use different notation from that we employed above in our explanation of F\&L.
...
\end{align*}
Under the assumptions of F\&L/ Mahajan,
\[
  \mathbb{E}\left\{ \left[
    \begin{array}{c}
    y - \kappa_1 - \theta_1 T \\
...
\right]\right\} = \mathbf{0}
\]
So we only require preliminary estimators of $\kappa_1$ and $\theta_1$.
Thus, compared to the preceding section $h^E$ becomes
\[
 h^E = \left[
 \begin{array}{c}
   y - \kappa_1 - \theta_1 T \\
 ...
 \right]
\]
and accordingly $H^E$ becomes
\[
  H^E =\mathbb{E}\left[
  \begin{array}{cc}
    \displaystyle \frac{\partial h^E}{\partial \kappa_1} &
    \displaystyle \frac{\partial h^E}{\partial \theta_1}
  \end{array}
\right] = \left[
\begin{array}{cc}
...
\right]
\]
and accordingly $M^E$ becomes
\[
  M^E = \mathbb{E}\left[
  \begin{array}{cc}
    \displaystyle \frac{\partial m^E}{\partial \kappa_1} &
    \displaystyle \frac{\partial m^E}{\partial \theta_1} 
  \end{array}
\right] = \left[
\begin{array}{cc}
...
Thus,
\begin{align*}
  B^E &= -M^E (H^E)^{-1} = \left[ \mathbb{E}(Tz) - \mathbb{E}(T)\mathbb{E}(z) \right]^{-1} 
\left[
\begin{array}{cc}
  \mathbb{E}[T] & 0 \\
  \mathbb{E}[Tz] & 0
...
\end{array}
\right] \end{align*}
and we see that the inverse of $H^E$ exists so long as $T$ is correlated with $z$.
The dimensions of $A$ change as follows:
\[
  \underset{(14 \times 24)}{A} = \left[
\begin{array}{cc}
  \underset{(14\times 14)}{\mathbf{I}} & \underset{(14 \times 10)}{B}
\end{array}
\right], \quad
\underset{(14\times10)}{B} = \left[
  \begin{array}{cc}
    \underset{(4\times 8)}{\mathbf{0}} & \underset{(4\times 2)}{\mathbf{0}} \\
    \underset{(8\times 8)}{B^I} & \underset{(8\times 2)}{\mathbf{0}} \\
    \underset{(2 \times 8)}{\mathbf{0}} & \underset{(2\times 2)}{B^E} \\
  \end{array}
\right]
\]
