...
\]
\todo[inline]{I checked this in our simulation just to be completely sure: it is indeed correct.}

\paragraph{Standard GMM form:}
To express this system in the standard GMM form $E[f(\mathbf{w},\theta)]=0$ we need to agument these moment equalities with expressions for the means of $z, y, y^2, T,$ and $yT$ as follows:
\begin{align*}
  \mathbb{E}\left[z - q  \right] &= 0\\
  \mathbb{E}\left[y - \mu  \right] &= 0\\
  \mathbb{E}\left[y^2 - s  \right] &= 0\\
  \mathbb{E}\left[yT - r  \right] &= 0\\
  \mathbb{E}\left[\left( zy - q\mu \right) - \left( \frac{\beta}{1 -\alpha_1} \right)\left( zT - qp \right)\right] &= 0\\
  \mathbb{E}\left[\left( zy^2 - qs \right) - \left(\frac{\beta}{1 - \alpha_1}\right)\left\{2\left(zyT - qr \right)- \beta \left( zT - qp \right) \right\}\right] &= 0
\end{align*}
%Adding these variables makes the system somewhat more ungainly.
%But it turns out to be equivalent to pretend that $\mu, q, p, r$ and $s$ are known and substitute the usual consistent estimators, as we now show.

Let $\mathbf{w} = (y, T, z)'$, $\varphi = (\beta, \alpha_1)'$, $\gamma = (q, \mu, s, r)'$ and $\theta = (\varphi', \gamma')'$ 
Similarly, let $g$ denote the moment conditions that involve $\beta$ and $\alpha_1$, let $h$ denote those that involve only $q, \mu, s$ and $r$ and let $f = (g', h')'$ denote the full set of moment conditions.
Then we have
\[
  \mathbb{E}[f(\mathbf{w};\theta)] = 
  \mathbb{E}\left[
  \begin{array}{l}
    g(\mathbf{w};\varphi, \gamma)\\
    h(\mathbf{w};\gamma)
  \end{array}
\right] = 0
\]
Now, let $F = \mathbb{E}\left[\nabla'_\theta f(\mathbf{w};\theta)\right]$.
Since $h$ does not involve $\varphi$ and $\nabla'_{\gamma}h = -\mathbf{I}$, we have
\[
  F =
  \left[
  \begin{array}{cc}
    \mathbb{E}\left\{\nabla'_{\varphi} g(\mathbf{w};\varphi,\gamma)\right\} & \mathbb{E}\left\{\nabla'_{\gamma}g(\mathbf{w};\varphi,\gamma)\right\}\\
    \mathbf{0} & -\mathbf{I}\\
  \end{array}
\right]
\equiv
...
\right]
\]
Now, under the standard regularity conditions, a GMM estimator based on the sample analogue $f_n(\theta)$ of $\mathbb{E}[f(\mathbf{w};\theta)]=0$ based on a weighting matrix $\widehat{W}\rightarrow_p W$ converges in distribution to
\[
  -(F'WF)^{-1}F'W M, \quad M\sim N(0, \Omega) 
\]
where $\sqrt{n}f_n(\theta_0) \rightarrow_d M$.
The present example, however, is just-identified which means that $F$ is square and hence
\[
  -(F'WF)^{-1}F'W = F^{-1}W^{-1}(F')^{-1}F'W = -F^{-1}
\]
Now, using the special structure of $F$ from above,
\[
  -F^{-1} = 
  \left[
  \begin{array}{cc}
...
\left[
\begin{array}{cc}
  -G_{\varphi}^{-1} & -G_{\varphi}^{-1}G_{\gamma}\\
  \mathbf{0} & \mathbf{I}
\end{array}
\right]
\]
To proceed further we'll need to calculate $G_\varphi$ and $G_\gamma$.
First, we re-write the $g$-block of moment conditions as
\begin{align*}
  \mathbb{E}\left[zy - \left( \frac{\beta}{1 -\alpha_1} \right) zT + qp \left( \frac{\beta}{1 - \alpha_1} \right) - q\mu\right] &= 0\\
  \mathbb{E}\left[zy^2 - 2\left(\frac{\beta}{1 - \alpha_1}\right)zyT + \left( \frac{\beta^2}{1 - \alpha_1} \right) zT + 2qr\left( \frac{\beta}{1 - \alpha_1} \right) - qp\left( \frac{\beta^2}{1 - \alpha_1} \right) - qs\right] &= 0
\end{align*}
Now, we have
\[ 
  \left[
...
\right] = 
  \left[
  \begin{array}{cc}
    \displaystyle -\left(\frac{zT - qp}{1 - \alpha_1}\right) & \displaystyle \frac{\beta(zT - qp)}{(1 - \alpha_1)^2} \\ \\
...
\[
  ...
  \begin{array}{cc}
    ...
    \displaystyle -2\left\{ \frac{\mbox{Cov}(yT,z) - \beta\mbox{Cov}(z,T)}{1 - \alpha_1} \right\} & \displaystyle \frac{ \beta^2\mbox{Cov}(z,T) - 2\beta \mbox{Cov}(yT,z)}{(1 - \alpha_1)^2} 
  \end{array}
\right]
\]
...
\left[
\begin{array}{cccc}
    \displaystyle \left( \frac{\beta}{1 - \alpha_1}  - \mu \right)  & -q & 0 & 0 \\
    \displaystyle\left( \frac{\beta}{1 - \alpha_1} \right)(2r - \beta p) - s & 0 & -q & \displaystyle \frac{2\beta q}{1 - \alpha_1}
\end{array}
\right]
\end{align*}
\todo[inline]{Now all that remains is to check the preceding derivatives and then calculate $-G_\varphi^{-1}$ and $-G_{\varphi}^{-1}G_\gamma$.}
