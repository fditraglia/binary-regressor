...
\begin{itemize}
  \item For simplicity, suppress dependence on $z$. By Theorem 1 and A4,
    \[
      \tau(v_k) M\left[ b_0, b_1, r(v_k) \right] = \tau(v_0)M\left[ b_0, b_1, r(v_k) \right]
    \]
...
\item If $V$ only took on two values, we could not identify $b_0$ and $b_1$ without burther restrictions. However if either $b_0$ or $b_1$ were known, e.g.\ known to be zero as in a one-sided mis-classification setting, then we could identify the model using only a binary $V$.
\end{itemize}


\subsection*{Old Lewbel Notes}
...
\begin{align*}
  ...
  \beta(x) &= m(1,\mathbf{x}) - m(0,\mathbf{x})\\
...
\begin{lem}[The $\eta_3$ equation]
  \label{lem:eta3}
\end{lem}
\begin{proof}[Proof of Lemma \ref{lem:eta3}]
\end{proof}

\begin{cor}[Identification of $\theta_1, \theta_2, \theta_3$]
...
\begin{proof}[Proof of Theorem \ref{thm:main_ident}]
  ...
  Substituting for $\alpha_0$ analogously gives a quadratic in $(1 - \alpha_1)$ with \emph{identical} coefficients.
...
Solving,
  ...
  \begin{align*}
    3(1 + \alpha_0 - \alpha_1)^2 &= 3\left[ 1 + (\alpha_0 - \alpha_1) \right]^2 = 3\left[ 1 + 2(\alpha_0 - \alpha_1) + (\alpha_0 - \alpha_1)^2 \right]\\
    &= 3 + 6\alpha_0 - 6\alpha_1 + 3 \alpha_0^2 + 3 \alpha_1^2 - 6\alpha_0\alpha_1 
  \end{align*}

  \begin{align*}
    2\left[ (1 - \alpha_0 - \alpha_1)^2 + 6\alpha_0(1 - \alpha_1) \right]&= 2\left\{ \left[ 1 - (\alpha_0 + \alpha_1) \right]^2 + 6\alpha_0(1 - \alpha_1) \right\}\\
  ...
  \begin{align*}
    3\theta^2_2 - 2\theta_1 \theta_3 &= \theta_1^4 \left\{ 3(1 + \alpha_0 - \alpha_1)^2 - 2 \left[ (1 - \alpha_0 - \alpha_1)^2 + 6 \alpha_0 (1 - \alpha_1) \right] \right\}\\
