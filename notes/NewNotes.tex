...
\begin{proof}[Proof of Lemma \ref{lem:eta2}]
  Since $z$ is binary, $\mbox{Cov}(w,z|\mathbf{x}) = q(\mathbf{x})\left[ 1 - q(\mathbf{x}) \right] \Delta_w(\mathbf{x})$ for any random variable $w$, where $\Delta_z \mathbb{E}(w|\mathbf{x}) \equiv \mathbb{E}(w|\mathbf{x},z=1) - \mathbb{E}(w|\mathbf{x},z=0)$.
Hence it suffices to show that 
\[
  \Delta_z \mathbb{E}(y^2|\mathbf{x}) = 2 \Delta_z \mathbb{E}(Ty|\mathbf{x}) \theta_1(\mathbf{x}) - \Delta_{z}\mathbb{E}(\mathbf{x})\theta_2(\mathbf{x}).
\]
...
\begin{proof}[Proof of Corollary \ref{cor:theta_ident}]
  ...
  Lemmas ??? yield a triangular linear system of equations in $\theta_1, \theta_2, \theta_3$, namely
\begin{align*}
 \pi\theta_1 &= \eta_1 \\
