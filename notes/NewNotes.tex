...
\section{June 1st, 2017 -- Second Moment Bounds}
...
(The $m^*_{tk}$ only enter the derivation as differences, so the $c$ disappears in any case.)
...
\end{align*}
we can write the second moment inequalities as
\begin{align*}
  \mathbb{E}\left[ (1- p_k - \alpha_1) y^2 \mathbf{1}\left\{ z=k \right\}(1 - T - \alpha_1) - \left\{ \alpha_1 m_{1k} - (1 - \alpha_1)m_{0k} \right\}^2 \right] &>0 \\
  \mathbb{E}\left[ (p_k - \alpha_0) y^2 \mathbf{1}\left\{ z=k \right\}(T - \alpha_0) - \left\{ (1 - \alpha_0) m_{1k} - \alpha_0 m_{0k} \right\}^2 \right] &>0 
\end{align*}
...
If $\alpha_0 = 0$ then the RHS of Inequality \ref{ineq:a0} becomes zero while the LHS becomes $p_k^2 \sigma_{1k}^2$.
Thus, Inequality \ref{ineq:a0} is always satisfied when $\alpha_0 = 0$.
If $\alpha_0 = 1$, the RHS is again zero while the LHS becomes $(1 - p_k)^2\sigma_{0k}^2$.
Thus, inequality \ref{ineq:a0} is always satisfied when $\alpha_0 = 1$.
If instead $\alpha_0 = p_k$, then the RHS becomes $p_k^2(1-p_k)^2(\mu_{1k} - \mu_{0k})^2$ while the LHS is zero.
Thus, unless $\mu_{1k} = \mu_{0k}$, when $\alpha_0 = p_k$ Inequality \ref{ineq:a0} is violated.
Rearranging Inequality \ref{ineq:a0}, we have $A_k \alpha_0^2 + B^0_k \alpha_0 + C^0_k \geq 0$ where
\begin{align*}
...
\end{align*}
Since $A_k > 0$, the quadratic equation defined by $A_k \alpha_1^2 + B^0_k \alpha_1 + C_k^0 = 0$ opens upwards.



Notice that both $\alpha_0 = 0$ and $\alpha_0 =1$ satisfy the inequality.
In contrast $\alpha_0 = p_k$ does not: the left hand side becomes zero while the right hand side is \emph{strictly} positive.
This implies that the quadratic equation defining the boundary of the inequality crosses the $\alpha_0$-axis and thus has two real roots.
Moreover, one of these is strictly less than $p_k$.
Rearranging Inequality \ref{ineq:a0}, we have $A_k \alpha_0^2 + B^0_k \alpha_0 + C^0_k > 0$ where
\begin{align*}
  A_k &= p_k (1 - p_k)(\bar{y}_{1k} - \bar{y}_{0k})^2 + (1 - p_k) \sigma_{0k}^2 + p_k \sigma_{1k}^2\\
  B_k^0 &= - \left[ \sigma_{1k}^2 p_k(1 + p_k) + p_k (1 - p_k)\sigma_{0k}^2 + p_k (1 - p_k)(\bar{y}_{1k} - \bar{y}_{0k})^2 \right]\\
  C_k^0 &= p_k^2 \sigma_{1k}^2. 
\end{align*}
Since $A_k > 0$, the quadratic equation defined by $A_k \alpha_1^2 + B^0_k \alpha_1 + C_k^0 = 0$ opens upwards.
Since $\alpha_0 < p_k$ we need only consider the smaller of the two roots. 
Our bound imposes that $\alpha_0$ be strictly less than this quantity.
Analogous reasoning for Inequality \ref{ineq:a1}, using $1 - p_k$ rather than $p_k$, shows that the smaller of the two roots of $A_k \alpha_1^2 + B^1_k \alpha_1 + C^1_k = 0$ bounds $\alpha_1$ from above, where 
\begin{align*}
  A_k &= p_k (1 - p_k)(\bar{y}_{1k} - \bar{y}_{0k})^2 + (1 - p_k) \sigma_{0k}^2 + p_k \sigma_{1k}^2\\
  B^1_{k} &= - \left[p_k (1 - p_k) \sigma_{1k}^2 + (1 - p_k)(2 - p_k)\sigma_{0k}^2 + p_k(1 - p_k)(\bar{y}_{1k} - \bar{y}_{0k})^2 \right]\\
  C^1_{k} &= (1 - p_k)^2 \sigma_{0k}^2. 
\end{align*}
Notice that the coefficient of the squared term, $A_k$, is common to both quadratics.
Because the bounds for $\alpha_0$ and $\alpha_1$ hold for each $k$, we can take the tighter of each.
\end{document}
