...
In contrast, forming a grid for $(\alpha_0, \alpha_1)$ in two dimensions is essentially trivial, particularly since the parameter space is bounded: $\alpha_0, \alpha_1>0$ and we have assume $\alpha_0 + \alpha_1 < 1$.
\todo[inline]{Also say something about how eliminating $\theta_1$ gives us a very good idea of how to set up our grid, where to look, etc.\ since we know that we can make the second moment conditions exactly zero along a particular line in $(\alpha_0, \alpha_1)$-space.}
...
\right]\right\} = \mathbf{0}
\]
Note that, as in our moment conditions, these conditions are completely uninformative about $(\alpha_0, \alpha_1)$ when $\beta = 0$.
...
\begin{align*}
  ...
  \widehat{\kappa}_1 &= \bar{y} - \widehat{\theta}_1 \bar{T}
\end{align*}
...
\begin{align*}
  ...
  \widehat{\theta}_2 &= (\bar{yT} - \widehat{\kappa}_1 \bar{T}) - \widehat{\theta}_3 \bar{T}  
\end{align*}
...
\[
  \widehat{\beta} =  \widehat{\theta}_1 \sqrt{D} = \widehat{\theta}_1 \sqrt{\left( \frac{\widehat{\theta}_3}{\widehat{\theta}_1} \right)^2 + 4 \left( \frac{\widehat{\theta}_2}{\widehat{\theta}_1} \right)} = \mbox{sign}(\widehat{\theta}_1) \sqrt{\left(\widehat{\theta}_3\right)^2 +4 \widehat{\theta}_1 \widehat{\theta}_2}
\]
