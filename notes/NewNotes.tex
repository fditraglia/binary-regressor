...
The MR critical value $\widehat{c}_n(\lambda_0, 1 - \alpha)$ is calculated from \emph{two different approximations} to the sampling distribution of $T_n(\lambda_0)$: ``DR'' and ``PR,'' specifically
\begin{align*}
...
\paragraph{Definition of $T_n^{DR}(\lambda_0)$}
\begin{align*}
...
\paragraph{Definition of $T_n^{PR}(\lambda_0)$}
\begin{align*}
...
\todo[inline]{I need to think some more about how step 3 works in our setting. I think the set of minimizers can be a singleton. If the null is that the treatment effect is zero, then I think the set of minimizers corresponds to all $\alpha_0, \alpha_1$ within the bounds.}
