...
\begin{proof}[Proof of Proposition \ref{pro:eta2}]
  ...
  By Assumption ??? (Model) and the basic properties of covariance, 
\begin{align*}
...
\end{align*}
Now, by Assumptions ??? (IV 1st and 2nd moments), 
moments $\mbox{Cov}(\varepsilon,z) = \mbox{Cov}(\varepsilon^2,z) = 0$, and by Assumptions ??? (alphas don't depend on z) $\mbox{Cov}(TT^*,z) = (1 - \alpha_1)\mbox{Cov}(T^*,z)$ and $\mbox{Cov}(T^*,z) = \pi/(1 - \alpha_0 - \alpha_1)$.
Hence, 
\begin{align*}
...
Then, by iterated expectations and Assumption ??? (non-diff),
\begin{align*}
...
\end{align*}
But by Assumption ??? (IV validity), $\mathbb{E}[\varepsilon|z=k] = m_{1k}^*p_{k}^* + m_{0k}^*(1 - p_k^*)=0$ so that $m_{0k}^*(1 - p_k^*)= - m_{1k}^* p_k^*$.
The result follows.
\end{proof}
