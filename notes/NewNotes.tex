...
We can use the same basic idea when either $\alpha_0$ or $\alpha_1$ is know but nonzero.
...
Suppose we take differences to eliminate $\alpha_1$ and define:
\begin{align*}
  G(\alpha_0, \beta; \tau, \tau') &= 
  \left[\frac{\alpha_0 \Delta(\tau + \beta) + \widetilde{\Delta}_1(\tau) - \widetilde{\Delta}_1(\tau + \beta)}{\Delta(\tau)}\right]
  - \left[\frac{\alpha_0 \Delta(\tau' + \beta) + \widetilde{\Delta}_1(\tau') - \widetilde{\Delta}_1(\tau' + \beta)}{\Delta(\tau')}\right] \\
  &= \alpha_0 \left[ \frac{\Delta(\tau+ \beta)}{\Delta(\tau)} - \frac{\Delta(\tau' + \beta)}{\Delta(\tau')} \right] - \left[ \frac{\widetilde{\Delta}_1(\tau) - \widetilde{\Delta}_1(\tau+ \beta)}{\Delta(\tau)} - \frac{\widetilde{\Delta}_1(\tau') - \widetilde{\Delta}_1(\tau'+ \beta)}{\Delta(\tau')}\right]
 \end{align*}

 where $\alpha_0$ and $\beta$ are \emph{arbitrary} values, i.e.\ not necessarily equal to the true values $\alpha_0^o$ and $\beta^o$.


\paragraph{How to solve numerically?}
One possibility is brute force: just solve for a root in three dimensions.
But there may be a better approach that uses some properties of the functions $\Delta$ and $\widetilde{\Delta}_1$.
