...
\[
  \phi^{MR}_n (\lambda_0) \equiv \mathbf{1}\left\{ T_n(\lambda_0) > \widehat{c}_n^{MR}(\lambda_0, 1-\alpha) \right\}
\]
...
The MR critical value $\widehat{c}_n(\lambda_0, 1 - \alpha)$ is calculated from \emph{two different approximations} to the sampling distribution of $T_n(\lambda_0)$: ``DR'' and ``PR.''
Both the DR and PR approximations are constructed from $v^*_{n,j}(\theta)$ for $j = 1, \hdots, p$ defined as: 
\begin{align*}
  v^*_{n,j} &= \frac{1}{\sqrt{n}} \sum_{i=1}^{n} \left[ \frac{m_j(W_i,\theta) - \bar{m}_{n,j}(\theta)}{\widehat{\sigma}_{n,j}(\theta)} \right]\xi_i\\
  \xi_1, \cdots, \xi_n &\sim \mbox{ iid } N(0,1) \mbox{ independent of the data } W_1, \cdots, W_n.
\end{align*}
In particular, both the DR and PR approximations take the form
\[
  \inf_{\theta \in \widetilde{\Theta}}\left(\underbrace{\sum_{j=1}^p \left[ v_{n,j}^*(\theta) + \ell_j(\theta) \right]_{-}^2}_{\text{inequalities}} + \underbrace{\sum_{j=p+1}^{k}\left\{   v^*_{n,j}(\theta) + \ell_j(\theta)\right\}^2}_{\text{equalities}}  \right)
\]
where $v^*_{n,j}(\theta)$ is as defined in the preceding pair of displayed equations, $\widetilde{\Theta}$ is some set, and $\ell_j(\theta)$ is a \emph{slackness function}.
The difference between DR and PR is the choice of $\widetilde{\Theta}$ and $\ell_j(\theta)$.
