...
Recall that if $\varphi(\omega)$ is the characteristic function of $Y$ then $\varphi^{(r)}(0) = i^r E[Y^r]$ provided that the expectation exists where $\varphi^{(r)}$ denotes the $r$th derivative of $\varphi$.
Hence, $E[Y^r]=(-i)^r \varphi^{(r)}(0)$.
Now, Leibniz's formula for the $r$th derivate of a product of two functions $f$ and $g$ is:
\begin{align*}
  (fg)^{(r)} = \sum_{s=0}^r {r \choose s} f^{(r)}g^{r-s}
\end{align*}
which follows by induction from the product rule.
Applying this to Equation \ref{eq:CharacteristicSpecial} after pre-multiplying each side by $(-i)^r$ gives
\end{document}
