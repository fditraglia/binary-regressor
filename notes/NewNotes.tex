...
\section{Frank's New Stuff from September}

\subsection{Is the Treatment Effect Identified when $Y$ is Binary?}
Let $\pi_{tz} = P(Y=1|T=t, Z=z)$ and $\pi^*_{tz} = P(Y|T^*=t, Z=z)$.
...
For the first, let $q^*_t = P(Z=1|T^*=t)$. 
We have $q_0^* = q(1 - p_1 - \alpha_1)/(1 - p - \alpha_1)$ and $q_1^* - q(p_1 - \alpha_0)/(p - \alpha_0)$.
And by our usual argument,
\begin{align*}
...
and similarly
\begin{align*}
  P(Y|T^*=1) &= \left( \frac{1 - \alpha_0}{p-\alpha_0} \right) p P(Y|T=1) - \frac{\alpha_0}{p - \alpha_0}(1 - p) P(Y|T=0)\\
  P(Y|T^*=0) &= \left( \frac{1 - \alpha_0}{p-\alpha_0} \right) p P(Y|T=1) - \frac{\alpha_0}{p - \alpha_0}(1 - p) P(Y|T=0)
\end{align*}

\subsection{Is there a LATE interpretation of our earlier results?}
Let $J \in \left\{ a, c, d, n \right\}$ index an individual's \emph{type}: always-taker, complier, defier, or never-taker.
Let $Y(1)$ indicate the potential outcome when $T^*=1$ and $Y(0)$ indicate the potential outcome when $T^*=0$.
...
\begin{align*}
  ...
  \mathbb{E}\left( Y| T^* = 1, z = 1 \right) &= \mathbb{E}\left[ Y(1)|J=a \right]\\
  \mathbb{E}\left( Y| T^* = 0, z = 0 \right) &= \mathbb{E}\left[ Y(0)|J=n \right]
\end{align*}

\paragraph{LATE Version of Theorem 2 from the Draft}
\begin{align*}
...
\begin{align*}
  ...
  &= (1 - \alpha_1) \left\{ \frac{\mathbb{E}\left[ Y(1) - Y(0)|J=c \right](p_1 - p_0)}{1 - \alpha_0 - \alpha_1} (p_1 - p_0) \right\} + (p_1  - p_0) \mathbb{E}\left[ Y(0)|J=c \right]
\end{align*}
...
\]
Rearranging, this in turn is equvalient to
\[
  \mbox{Var}\left[ Y(1)|J=c \right] = \mbox{Var}\left[ Y(0)|J=c \right]
\]
which is a fairly strong restriction, even though it does not technically rule out heterogeneous treatment effects.
