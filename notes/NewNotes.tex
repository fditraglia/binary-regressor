...
Only in the non-generic case where $\mu_{0k} = \mu_{1k}^2$ \emph{and} $\sigma^2_{0k} = \sigma_{1k}^2$ for all $k$ do the weak bounds and second moment bounds coincide.
...
\section{June 3, 2017: Inference Procedure (no covariates)}
...
\]
where $\widehat{\boldsymbol{\gamma}}_0 = \widehat{\boldsymbol{\gamma}}(\theta_0)$ is preliminary estimator of the strongly identified parameters $\boldsymbol{\gamma}$ constructed from the moment equalities $\mathbb{E}\left[ h(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}) \right]=0$.
We define $\boldsymbol{\theta}' = (\theta_1, \theta_2, \theta_3)$ where
\begin{align*}
  \theta_1 &= \beta/(1 - \alpha_0 - \alpha_1)\\
  \theta_2 &= \theta_1^2 \left[ 1 + (\alpha_0 - \alpha_1) \right]\\
  \theta_3 &= \theta_1^3\left[ \left( 1 - \alpha_0 - \alpha_1 \right)^2 + 6\alpha_0\left( 1 - \alpha_1 \right) \right]
\end{align*}
and $\boldsymbol{\gamma}' = (\mathbf{p}', \boldsymbol{\nu}', \boldsymbol{\kappa}')$ where
  $\mathbf{p}'= (p_0, p_1)$,  $\boldsymbol{\nu}' = (\nu_{00}, \nu_{01}, \nu_{10}, \nu_{11})$ and $\boldsymbol{\kappa}' = (\kappa_1, \kappa_2, \kappa_3)$.
The moment functions $m^E$ are defined as 
\begin{align*}
  m^E(W_i, \boldsymbol{\theta}, \boldsymbol{\gamma}) &=  \mathbf{u}_i(\boldsymbol{\theta}, \boldsymbol{\gamma}) z_i\\
  \mathbf{u}_i(\boldsymbol{\theta}, \boldsymbol{\gamma}) &= 
  \left[
  \begin{array}{l}
 y_i - \kappa_1 - \theta_1 T_i\\
...
\right]
\end{align*}
while the moment functions $h$ for the preliminary estimator of $\boldsymbol{\gamma}$ are
\begin{align*}
  h(W_i, \boldsymbol{\theta}, \boldsymbol{\gamma}) &= \left[
  \begin{array}{l}
    h_p(W_i, \boldsymbol{\gamma})\\
    h_\nu(W_i, \boldsymbol{\gamma}) \\
    \mathbf{u}_i(\boldsymbol{\theta}, \boldsymbol{\kappa})
  \end{array}
\right]\\
h_p(W_i, \boldsymbol{\gamma}) &= \left[
\begin{array}{l}
  T_i (1-z_i)/(1-q) - p_0\\
  T_i z_i / q - p_1
\end{array}
\right] \\
h_\nu(W_i, \boldsymbol{\gamma}) &= \left[
\begin{array}{l}
...
\right]\\
\end{align*}
Finally, the inequality moment functions $m^I$ are
\begin{align*}
...
\left[
\begin{array}{r}
   (p_0 - \alpha_0) y_i^2 (1 - z_i)(T_i - \alpha_0) - \left\{ (1 - \alpha_0) \nu_{10} - \alpha_0 \nu_{00} \right\}^2 \\ 
   (p_1 - \alpha_0) y^2_i z_i(T_i - \alpha_0) - \left\{ (1 - \alpha_0) \nu_{11} - \alpha_0 \nu_{01} \right\}^2 \\
  (1- p_0 - \alpha_1) y^2 (1-z_i)(1 - T_i - \alpha_1) - \left\{ \alpha_1 \nu_{10} - (1 - \alpha_1)\nu_{00} \right\}^2 \\
  (1- p_1 - \alpha_1) y_i^2 z_i(1 - T_i - \alpha_1) - \left\{ \alpha_1 \nu_{11} - (1 - \alpha_1)\nu_{01} \right\}^2 
\end{array}
\right]
\end{align*}
...
Notice that the preliminary estimators of $\boldsymbol{p}$ and $\boldsymbol{\nu}$ do not depend on the null hypothesis $\boldsymbol{\theta}_0$.
...
Because our estimator of $\boldsymbol{\gamma}$ is just identified, the quantity required to calculate the adjusted covariance matrix estimator is $B = -MH^{-1}$ where 
\begin{align*}
  H &= \mathbb{E}\left[ \frac{\partial h(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\gamma}'} \right] - \mathbf{I}_9\\
  M &= \mathbb{E}\left[ \frac{\partial m(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\gamma}'} \right]  =
  \left[
  \begin{array}{c}
    \partial m^I(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)/\partial \boldsymbol{\gamma}' \\ 
    \partial m^E(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)/\partial \boldsymbol{\gamma}' 
  \end{array}
\right]  \\
\mathbb{E}\left[\frac{\partial m^I(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\gamma}'}\right] &=  
  \left[
  \begin{array}{c}
    \partial m^I_1(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)/\partial \boldsymbol{\gamma}' \\ 
    \partial m^I_2(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)/\partial \boldsymbol{\gamma}' 
  \end{array}
\right]  \\
\mathbb{E}\left[\frac{\partial m^I_1(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\gamma}'}\right] &=  
\left[
\begin{array}{rr}
  \mathbf{I}_2 & \mathbf{0}_{2 \times 7} \\
  -\mathbf{I}_2 & \mathbf{0}_{2\times 7}
\end{array}
\right] \\
\mathbb{E}\left[\frac{\partial m^I_2(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\gamma}'}\right] &=  
\mathbb{E}
\left[
\begin{array}{ccc}
...
\displaystyle \frac{\partial m^I_2(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\kappa}'}
\end{array}
\right]\\
\mathbb{E}\left[\frac{\partial m^I_2(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \mathbf{p}'} \right]
 &= \left[
\begin{array}{cc}
  y_i^2(1-z_i)(T_i - \alpha_0) & 0 \\
  0 & y_i^2 z_i(T_i - \alpha_0) \\
  -y_i^2(1-z_i)(1 - T_i - \alpha_1) & 0 \\
  0 & -y_i^2 z_i(1 - T_i - \alpha_1) 
\end{array}
\right]\\
\mathbb{E}\left[\frac{\partial m^I_2(W_i, \boldsymbol{\theta}_0, \boldsymbol{\gamma}_0)}{\partial \boldsymbol{\kappa}'} \right]
 &= \left[
   \scriptsize
\begin{array}{cccc}
  2\alpha_0\left\{ (1 - \alpha_0)\nu_{10} - \alpha_0 \nu_{00} \right\} & 0 & -2(1 - \alpha_0)\left\{ (1 - \alpha_0) \nu_{10} - \alpha_0 \nu_{00} \right\} & 0 \\
  0 & 2\alpha_0 \left\{ (1 - \alpha_0)\nu_{11} - \alpha_0 \nu_{01} \right\} & 0 & -2(1 - \alpha_0)\left\{ (1 - \alpha_0)\nu_{11} - \alpha_0 \nu_{01}  \right\} \\
  ? & 0 & ? & 0 \\
  0 & ? & 0 & ? 
\end{array}
\right]\\
