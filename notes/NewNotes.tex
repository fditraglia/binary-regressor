...
\end{align*}
By Assumptions ??? (1--3 moment IV exog), $\mbox{Cov}\left[ (c + \varepsilon)^3,z \right] = 0$.
Thus, expanding and simplifying,
\begin{align*}
  ...
  \tau_2 &= c^2 \mbox{Cov}(T,z) + \beta(\beta + 2c) \mbox{Cov}(TT^*,z) + \mbox{Cov}(T\varepsilon^2,z) + 2c\,\mbox{Cov}(TT^*\varepsilon,z)
\end{align*}
From the proof of Proposition \ref{pro:eta2}, we have 
$\tau_1 = c\pi + \mbox{Cov}(T\varepsilon,z) + \beta\mbox{Cov}(TT^*,z)$
so that $\eta_3 = 3\tau_2 \theta_1 - 3 \tau_1 \theta_2 + \pi \theta_3$ is trivially satisfied if $\beta = 0$.
Accordingly, we assume that $\beta \neq 0$ for the remainder of this argument.

Now, define $s^*_{tk} = \mathbb{E}[\varepsilon^2|T^*=t, z=k]$ and $p_k^* = \mathbb{P}(T^*=1|z=k)$.
By iterated expectations, Bayes' rule, and Assumption ??? (2nd moment non-diff),
\begin{align*}
...
\end{align*}
By Assumption ??? (2nd moment IV exog) $\mathbb{E}[\varepsilon^2|z=1] = \mathbb{E}[\varepsilon^2|z=0]$ and thus, by iterated expectations,
...
\end{equation}
Similarly, defining $m_{tk}^*$ as in the proof of Proposition \ref{pro:eta2},
\begin{equation}
  \mbox{Cov}(TT^*\varepsilon, z) = q(1 - q)(1 - \alpha_1)(p_1^* m_{1k}^* - p_0^* m_{10}^*) = (1 - \alpha_1) \mbox{Cov}(T^*\varepsilon, z) 
  \label{eq:TTstarEpsilon}
\end{equation}
using iterated expectations and Assumptions ??? (1st moment nondiff, alpha doesn't depend on z).
Now, as shown in the proof of Proposition \ref{pro:eta2}, 
\begin{align*}
...
\end{align*}
and combining these equalities with Equations  \ref{eq:TEpsilonSquared} and \ref{eq:TTstarEpsilon}, we find that

Defining the shorthand $\pi^*\equiv \mbox{Cov}(T^*,z)$, it follows that
\begin{align*}
  \tau_2 &= \mbox{Cov}\left[ (c + \varepsilon)^2 T, z \right] + 2 \beta \mbox{Cov}\left[ (c + \varepsilon)T^*,z \right] + \beta^2 \mbox{Cov}(TT^*,z)\\
  &= c^2 \mbox{Cov}(T,z) + \beta(\beta + 2c) \mbox{Cov}(TT^*,z) + \mbox{Cov}(T\varepsilon^2,z) + 2c\,\mbox{Cov}(TT^*\varepsilon,z)\\ 
  &= c^2\pi + \beta(\beta + 2c) (1 - \alpha_1)\mbox{Cov}(T^*,z)\\
  &= ??? \\
  &= \mbox{Cov}(T\varepsilon^2, z) + 2\left[ \beta + c(1 - \alpha_0 - \alpha_1) \right] \mbox{Cov}(T^*\varepsilon, z) + \left[ (c^2 + \beta^2)(1 - \alpha_1)  + c(2\beta - c \alpha_0) \right] \pi^*\\
  \tau_1 %&= c\pi + \mbox{Cov}(T\varepsilon,z) + \beta \mbox{Cov}(TT^*,z)\\ 
  %&= \left[c(1 - \alpha_0 - \alpha_1)\right] \mbox{Cov}(T^*,z) + (1 - \alpha_0 - \alpha_1)\mbox{Cov}(T^*\varepsilon,z) + \beta (1 - \alpha_1)\mbox{Cov}(T^*,z)\\ 
  %&= \left[c(1 - \alpha_0 - \alpha_1) + \beta(1 - \alpha_1)\right] \mbox{Cov}(T^*,z) + (1 - \alpha_0 - \alpha_1)\mbox{Cov}(T^*\varepsilon,z)\\ 
  %&= \left[(c + \beta)(1 - \alpha_1) - c \alpha_0\right] \mbox{Cov}(T^*,z) + (1 - \alpha_0 - \alpha_1)\mbox{Cov}(T^*\varepsilon,z)\\ 
  &= (1 - \alpha_0 - \alpha_1)\mbox{Cov}(T^*\varepsilon,z) + \left[(c + \beta)(1 - \alpha_1) - c \alpha_0\right]\pi^*
\end{align*}
and thus, after some algebra, we find that
\begin{align*}
  3\tau_2 \theta_1 - 3 \tau_1 \theta_2 + \pi \theta_3 &= 3\theta_1 \mbox{Cov}(T\varepsilon^2,z) + K_1 \mbox{Cov}(T^*\varepsilon,z) + K_2 \pi^* \\
  K_1 &= 6\theta_1 \left[ \beta + c(1 - \alpha_0 - \alpha_1) \right] - 3\theta_2 (1 - \alpha_0 - \alpha_1)\\
  K_2 &= 3\theta_1 \left[ (c^2 + \beta^2)(1 - \alpha_1) + c(2\beta - c\alpha_0) \right] - 3 \theta_2 \left[ (c + \beta)(1 - \alpha_1) - c\alpha_0 \right] + \theta_3
\end{align*}
Hence, it suffices to show that $\theta_1 \mbox{Cov}(T\varepsilon^2,z) = \beta \mbox{Cov}(T^*\varepsilon^2,z)$, that $K_1 = (3\beta^2 + 6c\beta)$ and that $K_2 = (\beta^3 + 3c\beta^2 + 3c^2\beta)$.
To establish the first of these claims, 

Finally,
\begin{align*}
  K_1 &= 6\theta_1 \left[ \beta + c(1 - \alpha_0 - \alpha_1) \right] - 3\theta_2 (1 - \alpha_0 - \alpha_1)\\
      &= 6\beta\theta_1 + 6c\theta_1 (1 - \alpha_0 - \alpha_1) - 3\theta_2 (1 - \alpha_0 - \alpha_1) \\
      &= 6\beta\theta_1 + 6c\beta - 3\theta_1^2(1 - \alpha_0 - \alpha_1) (1 + \alpha_0 - \alpha_1)\\
      &= 6\beta(c + \theta_1) - 3\beta \theta_1 (1 + \alpha_0 - \alpha_1)\\
      &= 6c\beta + 6 \beta\theta_1 - 3\beta \theta_1 (1 + \alpha_0 - \alpha_1)\\
      &= 6c\beta + 3 \beta\theta_1\left[2 -  (1 + \alpha_0 - \alpha_1)\right]\\
      &= 6c\beta + 3 \beta\theta_1\left[1 - (\alpha_0 - \alpha_1)\right]\\
\end{align*}
