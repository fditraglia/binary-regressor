...
set.seed(1983)
b_seq <- c(2, 1.5, 1, 0.5, 0.25, 0.2, 0.15, 0.1, 0.05, 0.01)
d <- 0.15
RF_seq <- b_seq * (1 - 2 * d)
CIresults <- sapply(b_seq, function(b) CI_sim(0.1, b, 1000, d))
CIout <- round(cbind(b_seq, RF_seq, t(CIresults)[,1:4]), 3)
colnames(CIout) <- c('b', 'RF', 'Cover RF', 'Cover GMM', 'Width RF', 'Width GMM')
library(Hmisc)
latex(CIout, file = '')
