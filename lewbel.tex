...
\todo[inline]{Still need to type in the part about the terms under the square root. Show that there are always two real solutions: one positive and one negative, both equal in magnitude. To tell which is the solution we need to impose the condition that the measurement error isn't too severe.}

Finally, given that $B_{0}=(1-\alpha_{1})\alpha_{0}$ and $B_{1}=(1-\alpha_{0})\alpha_{1}$,
we can solve for the mis-classification rates as follows:
\[
...
Given knowledge of $(\alpha_{0},\alpha_{1})$ we can solve for $\beta$ using $\beta= \beta_{OLS}/M(\alpha_0, \alpha_1, p)$. 
