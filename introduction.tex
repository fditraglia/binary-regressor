...
Our question is whether, and if so under what conditions, a discrete instrumental variable $z$ suffices to non-parametrically identify the causal effect $\beta(\mathbf{x})$ of $T^*$, when we observe not $T^*$ but a mis-classified binary surrogate $T$. 
...
We then turn our attention to inference, showing that both our model and related models from the literature suffer from a weak identification problem. 
...
But when $\beta(\mathbf{x})$ is small the ``mixture modes'' are nearly indistinguishable, making it impossible to reliably estimate these probabilities.
...
Combining them with the moment equalities that emerge from our identification result, we propose an identification robust procedure for uniformly valid inference using tools from the moment inequality literature.
...
Other than \cite{Mahajan}, the paper most closely related to our own is that of \cite{Ura}, who derives partial identification results for a local average treatment effect without assuming non-differential measurement error.
