...
In this case the confidence interval for $\beta$ is excessively wide, as we see from Table \ref{tab:bonf_width_1000} and Figure \ref{fig:bonf_3_1000}.\footnote{Of course the widths do shrink as sample size increases: see the results for $n = 2000$ in Appendix \ref{sec:simulation_supplement}.}
...
While a detailed theoretical investigation of this problem is beyond the scope of the present paper, we now explore the performance of a simple method for ``transitioning'' between robust and standard inference.
Although somewhat different in its details, this idea bears some similarity to
\todo[inline]{Cite that Isaiah Andrews paper}
