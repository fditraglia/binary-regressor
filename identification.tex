...
\begin{lem} Under Assumptions \ref{assump:model}, \ref{assump:misclassification} and \ref{assump:2ndMoment}, $\eta_2(\mathbf{x}) =  2\tau_1(\mathbf{x}) \theta_1(\mathbf{x}) - \pi(\mathbf{x})\theta_2(\mathbf{x})$,
  where $\pi(\mathbf{x}), \tau_1(\mathbf{x})$ and $\eta_2(\mathbf{x})$ are defined in Equation \ref{eq:observables}, and $\theta_1(\mathbf{x}),\theta_2(\mathbf{x})$ in Equations \ref{eq:theta1_def}--\ref{eq:theta2_def}.
  \label{lem:eta2}
\end{lem}
