...
While these restrictions can be very informative in practice, they do not yield point identification as we now show.

\begin{cor}
  Under Assumptions ??? the identified set for $\beta(\mathbf{x})$ contains at least two points: wald and truth.
  Hence, $\beta$ is not point identified.
\end{cor}

Say why we can't rule out Wald: no mis-classification is always a possibility.
Since our proofs have all been for an arbitrary $k$,turns out that preceding result holds even $z$ takes on an arbitrary (finite) number of values $k \in \left\{ 1, \cdots, K \right\}$ and Assumptions ??? are satisfied for all of them.
