...
Non-differential measurement error cannot excluded the possibility that there is no mis-classification because in this case it is trivial to construct the required mixtures.

Although we focus throughout this paper on the case of a binary instrument, one might wonder whether point identification can be achieved by increasing the support of $z$.
...
\todo[inline]{ Showed that weakening the joint exog to instrument mean exclusion implies not identified.
Derived sharp identified set.
One approach given the results of the preceding section is to just do partial identification.
Another approach, strengthen assumptions to get point identification.
We now consider simple set of extra assumptions that point identification.
Demand more from the data, but in an additively separable model they're not really much stronger that what we've already assumed.
Start by defining some notation: thetas, eta, tau etc.
Then give Assumption 2.5 and Lemma 2.3.
Then explain how this identifies the difference of alphas and hence gives point identification under one-sided mis-classification.
Need to add a corollary here.
Then give Assumption 2.6 and Lemma 2.4, followed by Corollary 2.4 and the main theorem.
End of section!}
