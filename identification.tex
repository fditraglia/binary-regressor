...
We begin by showing that this assumption generically shrinks the identified set from Theorem \ref{thm:sharpI}.


\todo[inline]{Need to make this clearer: the point should be that the assumption \emph{only} has the potential to rule out some values of $\alpha_0, \alpha_1$ and that it strictly improves at least one of the bounds if the mean restriction holds for at least one $k$. It improves both bounds if it holds for both.}

\begin{thm}
  Assumption \ref{assump:misclassification} (iii) strictly improves the bounds for $\alpha_0(\mathbf{x})$ and $\alpha_1(\mathbf{x})$ from Theorem \ref{thm:sharpI} if and only if
  $\mathbb{E}\left[y|\mathbf{x},T=0,z=k  \right] \neq \mathbb{E}\left[ y|\mathbf{x},T=1,z=k \right]$ for some $k = 0,1$.
  \label{thm:shrink}
\end{thm}



  The proof of this lemma forms the basis of the sharp identified set result in the next Theorem.
