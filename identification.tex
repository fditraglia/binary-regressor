...
Assumption \ref{assump:model}(i) is a restatement of the additively separable model from Equation \ref{eq:model}, which includes as a special case the linear model $y = c + \beta T^* + \mathbf{x}'\boldsymbol{\gamma} + \varepsilon$ that is pervasive in empirical economics. 
...
\end{align*}
%We impose the following conditions on the mis-classification process.
...
Assumption \ref{assump:misclassification}, or a variant thereof, is standard in the theoretical literature on mis-classification \citep{Mahajan,BBS,FL,Lewbel,hu2008} and in empirical studies that allow for measurement error in a binary or discrete variable \citep{KRS,hu2013,Batt}.
...
While Assumption \ref{assump:misclassification}(ii) is quite mild, Assumptions \ref{assump:misclassification} (i) and (iii) are fairly restrictive, as discussed by \cite{Bound2001}.
...
Just as the instrument validity condition, Assumption \ref{assump:model} (iii), is more plausible after conditioning on a rich set of exogenous controls, so too are assumptions regarding mis-classification. 
...
While Assumptions \ref{assump:2ndMoment} and \ref{assump:3rdMoment} may appear unfamiliar, we consider them to be natural in the context of an additively separable model in which one has already assumed that $\mathbb{E}[\varepsilon|\mathbf{x},z]=0$ and $\mathbb{E}[\varepsilon|\mathbf{x},z,T^*,T] = \mathbb{E}[\varepsilon|\mathbf{x},z,T^*]$ -- Assumptions \ref{assump:model} (iii) and \ref{assump:misclassification} (iii) from above.
%\footnote{If one wishes to weaken our Assumption \ref{assump:model} (i) to allow for some form of unobserved heterogeneity, our higher moment assumptions may impose additional restrictions.}
