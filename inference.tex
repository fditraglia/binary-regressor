...
These moment inequalities will be excluded when approximating the large-sample distribution of test statistic.
...
We now detail our procedure to construct a valid confidence interval for $\beta$.
...
\begin{alg}[Bonferroni-based Confidence Interval for $\beta$] \mbox{}\\
    ...
    \item Construct the $(1 - \delta)\times 100\%$ Bonferroni-based confidence interval $\left[ \underline{\beta}(\delta), \overline{\beta}(\delta) \right]$ for $\beta$.
      \begin{enumerate}[(i)]
        ...
        \item Set $\underline{\beta}(\delta) = \min \left\{ \underline{s}(\delta_1) \underline{\theta}_1(\delta_2), \overline{s}(\delta_1)\underline{\theta}_1(\delta_2) \right\}$.
        \item Set $\overline{\beta}(\delta) = \min \left\{ \underline{s}(\delta_1) \overline{\theta}_1(\delta_2), \overline{s}(\delta_1)\overline{\theta}_1(\delta_2) \right\}$.
      \end{enumerate}
  \end{enumerate}
...
\begin{thm}
  ...
  Then, under standard regularity conditions, the confidence interval for $\beta$ described in Algorithm \ref{alg:Bonferroni} has asymptotic coverage probability of no less than $1 - (\delta_1 + \delta_2)$ as $R,N,n\rightarrow \infty$ uniformly over the parameter space.
  \label{thm:uniform}
\end{thm}

Theorem \ref{thm:uniform} is effectively a corollary of Theorem 1 from \cite{AndrewsSoares}, which establishes the uniform asymptotic validity of the GMS test, and \ref{lem:prelim}, which accounts for preliminary estimation of $\boldsymbol{\gamma}$ and $\mathbf{q}$.
Given iid observations $\mathbf{w}_i$, the only substantive condition required for Theorem \ref{thm:uniform} is the joint asymptotic normality of $\sqrt{n} \,\bar{m}_n(\boldsymbol{\vartheta}_0, \boldsymbol{\tau}_0)$ and $\bar{h}_n(\boldsymbol{\vartheta}_0, \boldsymbol{\tau}_0)$, where $\bar{h}_n$ denotes the sample analogues for full set of auxiliary moment conditions $(h^I, h^E)$ defined in Section \ref{sec:prelim}.
For more details see Appendix A3 of \cite{AndrewsSoares}.
For some discussion of the regularity conditions required for Lemma \ref{lem:prelim}, see Section \ref{sec:prelim}.



\todo[inline]{Talk about how you could proceed for the exogenous case (same moment inequalities!) or if you only wanted to use the inequalities and do partial identification}
