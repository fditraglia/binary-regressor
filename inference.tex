...
Collecting these for all $(t,k)$ gives $\widehat{\mathbf{q}}$.
Now, define
\begin{equation}
...
$\widehat{\mathbf{q}} \rightarrow_p \mathbf{q}_0$ where $\mathbf{q}_0$ satisfies the just-identified collection of unconditional moment equalities $\mathbb{E}[h^I(\mathbf{w}_i, \boldsymbol{\vartheta}_0,\mathbf{q}_0)] = \mathbf{0}$.
...
\begin{array}{cccc}
  -1 & 0 & 0 
  & \left(\frac{\partial \boldsymbol{\psi}_1}{\partial \theta_1}\right)'\mathbb{E}[\mathbf{w}]\\
   0 & -1 & 0 &  \left(\frac{\partial \boldsymbol{\psi}_2}{\partial \theta_1}\right)'\mathbb{E}[\mathbf{w}]\\
   0 & 0 & -1 & \left(\frac{\partial \boldsymbol{\psi}_3}{\partial \theta_1}\right)'\mathbb{E}[\mathbf{w}]\\
   -\mathbb{E}[z_i] & 0 & 0 &  \left(\frac{\partial \boldsymbol{\psi}_1}{\partial \theta_1}\right)'\mathbb{E}[\mathbf{w}z]
\end{array}
\right]
\]
\end{lem}

\todo[inline]{Need to define the $\bar{h}_n$ and $\bar{m}_n$. Just say defined analogously to what we had above. Add some kind of assumptions to the lemma, add at least a proof sketch to the appendix, and discuss the result a bit. Allows us to adjust the asymptotic variance matrix given a joint CLT. Talk about the existence of $B^I$ and $B^E$: by direct calculation, the determinant of $H^E$ equals $\mbox{Cov}(T,z)$ so this is always invertible. Talk about what happens to $B^I$ when $\alpha_1 = 0$ but how this doesn't matter because the corresponding inequalities are trivially satisfied when testing that null so we just drop the corresponding conditions.}
