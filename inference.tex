...
where
\begin{equation}
  \widehat{\mathcal{V}}_n(\boldsymbol{\vartheta}_0, \widehat{\boldsymbol{\tau}}_0) \equiv \frac{1}{n} \sum_{i=1}^n
\left[
\begin{array}{c}
...
\begin{equation}
  S(m, \Sigma) = \sum_{j=1}^{J_I}\min\left\{ 0, 
    \left(m_j / \sigma_j\right)^2 \right\} + \sum_{j=J_1 + 1}^{J} \left(m_j/\sigma_j \right)^2
    \label{eq:MMM}
\end{equation}
...
\begin{alg}[GMS Test for $\alpha_0$ and $\alpha_1$ ]
  ...
  \begin{enumerate}
    \item Calculate the test statistic $T_n(\boldsymbol{\vartheta}_0)$: 
      \begin{enumerate}[(i)]
    \item Caclulate the preliminary estimators:  $\widehat{\boldsymbol{\tau}}_0 = \big( \widehat{\boldsymbol{\gamma}}'(\boldsymbol{\vartheta}_0), \widehat{\mathbf{q}}'(\boldsymbol{\vartheta}_0) \big)$, from Section \ref{sec:prelim}.
