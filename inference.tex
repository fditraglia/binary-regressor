...
\begin{alg}[Bonferroni-based Confidence Interval for $\beta$] \mbox{}\\
    ...
    simulations $\{\boldsymbol{\zeta}^{(r)}\}_{r=1}^R \sim \mbox{ iid } N_{J+K}(\mathbf{0}, \mathbf{I})$.
  \begin{enumerate}
    \item Construct a $(1 - \delta_1)\times 100\%$ joint confidence set $\mathcal{C}_\vartheta(\delta_1)$ for $\boldsymbol{\vartheta} = (\alpha_0, \alpha_1)'$. 
      \begin{enumerate}[(i)]
        \item Let $\Lambda_N = \left\{0, \frac{1}{N}, \frac{2}{N}, \hdots, \frac{N-2}{N}, \frac{N-1}{N}\right\}$ where $N>1$ is an integer. 
        \item Set $\Delta_N = \left\{(\alpha_0, \alpha_1) \in (\Lambda_N \times \Lambda_N) \colon \alpha_0 + \alpha_1 < 1 \right\}$.
        \item For each $\boldsymbol{\vartheta} \in \Delta_N$ calculate $\widehat{p}(\boldsymbol{\vartheta})$ using Algorithm \ref{alg:GMS}. 
        \item Set $\mathcal{C}_{\vartheta}(\delta_1) = \left\{ \boldsymbol{\vartheta} \in \Delta_N \colon \widehat{p}(\boldsymbol{\vartheta}) \geq \delta_1\right\}$.
      \end{enumerate}
    \item Do projection to get the set for $1 - \alpha_0 - \alpha_1$
    \item Construct a $(1 - \delta_2)\times 100\%$ confidence interval for $\theta_1$.
