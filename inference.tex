...
\begin{lem}
  Let $\bar{m}^I_{1,n}(\boldsymbol{\vartheta}) = n^{-1}\sum_{i=1}^n m_{1,n}^I(\mathbf{w}_i,\boldsymbol{\vartheta})$ and define $\bar{m}_{2,n}^I, \bar{m}_n^E, \bar{h}_n^I$ and $\bar{h}_n^E$ analogously.
...
Lemma \ref{lem:prelim} relates the sample analogues $\bar{m}_{2,n}^I$ and $\bar{m}_n^E$ evaluated at the preliminary estimators $\widehat{\mathbf{q}}(\boldsymbol{\vartheta}_0)$ and $\widehat{\boldsymbol{\gamma}}(\boldsymbol{\vartheta}_0)$ to their counterparts evaluated at the true parameter values $\mathbf{q}_0$ and $\boldsymbol{\gamma}_0$ under $H_0\colon \boldsymbol{\vartheta} = \boldsymbol{\vartheta}_0$.
...
A few lines of matrix algebra using the definitions of $(\boldsymbol{\psi}_1,\boldsymbol{\psi}_2,\boldsymbol{\psi}_3)$ from Equation \ref{eq:psi_def} and $\mathbf{w}_i$ from Section \ref{sec:problem} establish that the determinant of $H^E$ equals $\mbox{Cov}(z,T)$.
Hence, $B^E$ is well-defined if $z$ is a relevant instrument.
Provided that $\mathbf{q}$ is well-defined under $H_0\colon \boldsymbol{\vartheta} = \boldsymbol{\vartheta}_0$ and $\alpha_1 \neq 0$, $B^I$ is likewise well-defined.
If either of these conditions fails, however, we are in a case where some of the moment inequalities in $m_2^I$ are trivially satisfied and can be dropped (see Footnote \ref{foot:quantile}).
After removing the corresponding elements of $\mathbf{q}$ and $\mathbf{a}$, $B^I$ becomes well-defined.

\todo[inline]{Proof of Lemma \ref{lem:prelim} relies on mean value expansions: uses diff of certain functions or their expectations along with interiority. Need to at least comment on this although the functions we work with satisfy these conditions.}

Using Lemma \ref{lem:prelim}, we can reduce the dimension of 
