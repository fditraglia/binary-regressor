...
Since this point lies in the identified set from Theorem \ref{thm:sharpI}, it suffices to determine whether there exist valid conditional CDFs $F_{tk}^{0}, F_{tk}^{1}$ such that $F_{tk} = (1 - r_{tk}) F_{tk}^1 + r_{tk} F_{tk}^1$ for all $t,k$ and $\mathbb{E}[y|T,T^*,z] = \mathbb{E}[y|T^*,z]$.
Define the shorthand $\mu_{tk}^{t^*} = \mathbb{E}[y|T=t,z=k,T^*=t^*]$, $\mu_{tk} = \mathbb{E}[y|T=t,z=k]$, and $\mu_{k}^{t^*} = \mathbb{E}[y|z=k,T^*=t^*]$.
By Assumption \ref{assump:misclassification} (iii) $\mu_{tk}^{t*} = \mu_{k}^{t^*}$ for all $t = 0,1$.
Hence, by iterated expectations,
\begin{align*}
  \mu_{0k} &= (1 - r_{0k}) \mu_{k}^0 + r_{0k} \mu_{k}^1\\ 
  \mu_{1k} &= (1 - r_{1k}) \mu_{k}^0 + r_{1k} \mu_{k}^1.
\end{align*}
Now, $(\mu_{0k},\mu_{1k})$ are observed while $r_{0k}$ and $r_{1k}$ depend only on $p_k$, $\alpha_0$, and $\alpha_1$. Thus, at a given point $(c, \beta, \alpha_0, \alpha_1)$ in the identified set from Theorem \ref{thm:sharpI} the preceding equations form a linear system in two unknowns: $\mu_{k}^0$ and $\mu_{k}^1$.
...
\]
so that a unique solution exists if and only if $\alpha_0 \neq p_k$ \emph{and} $\alpha_1 \neq 1 - p_k$, in particular:
\begin{align*}
...
\end{align*}
Note that $\mu_{0k} = \mu_{1k}$ implies $\mu_{k}^0 = \mu_k^1$.

\todo[inline]{Remember to emphasize that $\mu_{k}^0,\mu_{k}^1$ are equal to $\mu_{tk}^0,\mu_{tk}^1$!}

\todo[inline]{Need to Add: (1) Explanation for why we know that only $\alpha_0, \alpha_1$ are constrained by non-diff, (2) Exactly how we build on step 2 of the earlier proof, (3) Deal with the ``non-generic'' cases, (4) make sure we're covering all the four cases from the earlier theorem. I think that (3) and (4) are related. In particular, I think any of the ``edge'' cases from the earlier proof are situations in which the non-diff assumption doesn't give you anything.}
