...
\begin{proof}[Proof of Theorem \ref{thm:sharpI}]
  To prove sharpness, we need to show that under Assumptions \ref{assump:model} and \ref{assump:misclassification} (i)--(ii) there exists a valid joint distribution for $(y,T,T^*,z)$ that is compatible with the observed joint distribution $(y,T,z)$ if and only if $(c, \beta, \alpha_0, \alpha_1)$ satisfy the conditions of the Theorem.
...
In the first step, we rely on the fact that Assumptions \ref{assump:model} (ii) and \ref{assump:misclassification} (i)--(ii) constrain only the distribution of $(T,T^*,z)$.
We show that, under these assumptions, it is only possible to construct a valid joint distribution for $(T,T^*,z)$ that is compatible with the observed distribution for $(T,z)$ if $\alpha_0 \leq p_k \leq 1 - \alpha_1$ for $k = 0,1$.
Next we show that, under Assumptions \ref{assump:model} (i) and (iii) it is only possible to construct a valid conditional distribution for $y$ given $(T,T^*,z)$ that is compatible with the observed conditional distribution of $y$ given $(T,z)$ if $\mathbb{E}[y|z=k] = c + \beta (p_k - \alpha_0)(1 - \alpha_0 - \alpha_1)$ for $k = 0,1$.
...
\]
Since $p(z)$ is observed, this decomposition shows that we can only construct a valid joint probability mass function $p(T^*,T,z)$ if we can construct valid \emph{conditional} probability mass functions $p(T|T^*)$ and $p(T^*|z)$.

so long as we construct valid \emph{conditional} probability mass function 
the preceding decomposition automatically yields a valid joint probability mass function $p(T^*,T,z)$.
...
For each of the remaining cases, we show that it is possible to construct the required distributions $F_{tk}^0, F_{tk}^1$ while satisfying our model assumptions if and only if $\mathbb{E}(y|z=k) = c + \beta(p_k - \alpha_0)/(1 - \alpha_0 - \alpha_1)$.
...
In the proofs of Lemma \ref{lem:eta2}, Lemma \ref{lem:eta3}, and Theorem \ref{thm:main_ident}, we use the shorthand $\pi \equiv \mbox{Cov}(T,z)$, $\eta_j \equiv \mbox{Cov}(y^j,z)$, and $\tau_j \equiv \mbox{Cov}(Ty^j,z)$ for $j = 1, 2, 3$.
