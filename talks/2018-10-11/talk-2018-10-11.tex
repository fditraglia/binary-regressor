...
\begin{frame}
  ...
  \note{\singlespacing\scriptsize
    \begin{itemize}
      \item This brings us to our second main contribution: the sharp identified set for $(\alpha_0, \alpha_1, \beta)$ under the baseline assumptions. The description of the set is fairly complicated, so I'm not going to show it on the slide. But the form that this set takes leads to two important results. First, the non-differential measurement error assumption \emph{generically} improves upon the weak bounds. Second, under the baseline assumptions $\beta$ is \emph{never} point identified, regardless of how many different (discrete) values the instrument takes on.
      \item Some intuition: the true $\beta$ always lies within the identified set by definition. It turns out that $\alpha_0 = \alpha_1 = 0$ implies that the mixing probabilities $r_{tk}$ are all either zero or one. But in this case the mixtures are trivial, so we can simplly set $F = H$. Hence, the IV estimand $\beta / (1 - \alpha_0 - \alpha_1)$ always lies in the sharp identified set.
      \item Corollary: everything I've said so far concerns an additively separable model. But in fact, bounds we derive under the baseline assumptions remain valid if we re-state our assumptions so that they involve a LATE model. The bounds we describe in our paper may not be sharp in a LATE model, however, because the LATE assumptions themselves have testable implications that we do not impose as we are primarily interested in the additively separable case.
      \item So now what? The sharp bounds can actually be quite informative in practice and they are simple to compute.
        But we know that they do not point identify $\beta$. So if the baseline assumptions aren't enough, are there slightly stronger but still plausible assumptions that allow us to point identify $\beta$? The answer turns out to be yes.
    \end{itemize}
  }%
