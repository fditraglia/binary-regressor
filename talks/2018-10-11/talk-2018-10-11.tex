...
\end{block}
  ...
  \begin{alertblock}{Necessary and Sufficient Condition if $F$ is Continuous}
   ...
   \]
    \scriptsize
    \begin{align*}
     \underline{\mu}(F,p) &\equiv \int_{-\infty}^{F^{-1}(p)} \frac{x}{p}\; f(x) \; dx\\
     \overline{\mu}(F,p) &\equiv \int_{F^{-1}(1-p)}^{+\infty} \frac{x}{p} \; f(x)\; dx
   \end{align*}
  \end{alertblock}
...
\begin{alertblock}{Our Solution}
  ...
  \begin{itemize}
    \item Sharp identified set result implies a number of \emph{inequality} moment restrictions that remain informative even if $\beta$ is small or zero. \hyperlink{INEQ_APPEND}{\beamergotobutton{more}}
...
\begin{frame}
  ...
  \begin{block}{Special Structure}
    \begin{itemize}
      ...
      \item Strong instrument $\implies$ inference for $\theta_1$ is standard.
