...
\note{\singlespacing\scriptsize
  \begin{itemize}
    ...
    \item This slide and the thext one detail what I will call the ``baseline'' assumptions, which I will maintain through the talk.
    \item The first part of the baseline assumptions concern the model and instrument: all that this slide says is that if $T^*$ were observed, then the model would be identified.
    \item In particular, these conditions are simply the usual instrument relevance and validity conditions in the model with the true, unobserved regressor $T^*$.
  \end{itemize}
}%
...
\begin{frame}
  ...
  \note{\singlespacing\tiny
    \vspace{-2.5em}
    \begin{itemize}
      \item 2nd part of the baseline assumptions concerns measurement error process. 
        First need some notation: mis-classification probs.\ $\alpha_0$ and $\alpha_1$. 
        Two possible errors.
        \emph{Upwards} mis-classification: observe $T=1$ when truth is $T^*1$.
        This occurs with prob.\ $\alpha_0$.
        \emph{Downwards} mis-classification: observe $T=0$ when truth is $T^*=1$.
        This occurs with prob.\ $\alpha_1$.
        Convention uses subscripts to indicate the value of \emph{truth}: $\alpha_0$ is mis-classification prob.\ when $T^*=0$ ($\uparrow$) and $\alpha_1$ when $T^*=1$ ($\downarrow$).  
        So far notation: now restrictions
      \item First: given $x$ the mis-class.\ rates don't depend on z. Restrictive, but hard to make progress without it.
        How reasonable depends on $\mathbf{x}$ and specific setting.
        Plausible: smoking mothers didn't know if they had the real or placebo patch.
      ...
      \item Second: $Cor(T,T^*)>0 \iff \alpha_0 + \alpha_1 < 1$. Mild assumption. I'll say more in a few slides.
      \item Third: \emph{non-diff} assumption. Stated in terms of epsilon, but what this really requires is conditional mean of Y doesn't depend on $T$ given $(\mathbf{x},z,T^*)$. Plausibility depends on the situation and the controls in $\mathbf{x}$. Example of what this rules out: ``returns to lying.'' E.g.\ $Y=\log(\mbox{wage})$, $T^*=$ true college dummy, and $T=$self-report of college. If employers can't perfectly observe credentials, there could be a \emph{direct} effect of $T$ on $y$ even after controlling for $T^*$. Working on a paper based on this example with Arthur Lewbel.
    \end{itemize}
  }%
...
\note{\singlespacing \scriptsize
  \begin{itemize}
    \item Describe the picture. Explain how the upper-right corner of the blue rectangle corresponds to the reduced form estimator and the lower-left to the IV estimator. Explain $\alpha_0 + \alpha_1 < 1$ in relation to the gray rectangle and red line.
    ...
    \item To answer this, derive sharp identified set under baseline assumptions: new to the literature.
      Important even if our main concern is point identification: while we showed a flaw in Mahajan's proof, we did \emph{not} demonstrate
...
\begin{frame}[t,noframenumbering]
  ...
  \note{\singlespacing\scriptsize
    \begin{itemize}
      \item Suppress dependence on $\mathbf{x}$. Study conditional dist of $y|T,T^*,z$. Unobserved but related to dist of $y|T,z$ via a mixture model. Mixing probs are $r_{tk}$. These turn out to be a function of $(\alpha_0, \alpha_1)$ and observables only. Shorthand: $z_k$ denotes $z=k$.
...
\end{figure}
  ...
  \note{\singlespacing\scriptsize
    \begin{itemize}
      ...
      \item In contrast, if $p=1$ then $\mu$ must \emph{equal} the mean of the observed distribution $F$, in this case $-0.8$, since this corresponds to a degenerate mixture in which $F=H$.
      \item So how does this relate to our original problem? Remember that we observe the distribution of $y|T,z$ which is related to the unobserved distribution of $y|T,T^*,z$ via a mixture model. The mixing probability depends only on observables and $(\alpha_0, \alpha_1)$ as do the means of the mixture components. Hence, some values of $(\alpha_0, \alpha_1)$ are incompatible with the mixture model.
        This in turn restricts $\beta$ through its relationship to the IV estimand. In fact we have \emph{joint} restrictions for all $(t,k)$ so the book-keeping is complicated, but the basic intuition is exactly as I've shown you in this simple mixture of normals example.
    \end{itemize}
  }%
...
\begin{frame}
  ...
  \note{\singlespacing\scriptsize
    \vspace{-2em}
    \begin{itemize}
      \item Second main contribution: sharp identified set for $(\alpha_0, \alpha_1, \beta)$ under the baseline assumptions. The description of the set is fairly complicated, so I'm not going to show it on the slide. But the form that this set takes leads to two important results. First, the non-differential measurement error assumption \emph{generically} improves upon the weak bounds. Second, under the baseline assumptions $\beta$ is \emph{never} point identified, regardless of how many different (discrete) values $z$ takes.
      ...
      \item What now? Sharp bounds quite informative in practice, but they do not point identify $\beta$.
        Baseline assumptions aren't enough. Are there slightly stronger but still plausible assumptions that allow us to point identify $\beta$? Yes! 
    \end{itemize}
  }%
...
\end{align*}
  ...
  \note{Note that $\beta = 0$ iff $\theta_1 = \theta_2 = \theta_3 = 0$.}
...
\begin{frame}
  ...
  \note{Notice that the corollary implies that $\beta$ is point identified if mis-classification is one-sided, as it might well be in the smoking example.}
...
\begin{frame}
  ...
  \note{Explain what not asymptotically conservative means. Explain what projection is and why it's conservative and computationally intensive.}
