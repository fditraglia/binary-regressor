...
\begin{frame}
  ...
  \begin{block}{Intuition}
    No mis-classification $\Rightarrow r_{tk} = 0 \mbox{ or } 1$ and we can always form a valid mixture in this case. Hence the Wald estimand is always in the sharp identified set for $\beta$. 
  \end{block}

  \begin{block}{Point identification from stronger assumptions?}
  \end{block}

  \note{Also want to point out that the restrictions from non-differential measurement error can be very informative in practice! Now transition to point identification argument.}
...
\begin{frame}
  ...
  MENTION THE ANGRIST ARGUMENT BUT DON'T NEED TO CITE ANGRIST!\ldots
...
LINK TO A SLIDE SHOWING FULL CONTINUUM OF MOMENTS
